...
class MCControlAgent:
    ...
    def gen_eps(self, env, policy):
        ...
        while True:
            action = np.random.choice(2, p=policy[obs[0], obs[1], int(obs[2])])

            # wende die Aktion auf die Umgebung an
            next_obs, reward, terminated, truncated, _ = env.step(action)

            # speichere das Tupel (Zustand, Aktion, Belohnung) in der Episode
    ...
    def on_policy_control(self, env, n_eps, policy, eps=1.0, eps_decay=0.9999):
        ...
        for _ in range(n_eps):
            ...
            # Firstly, we simulate an episode and save all states and rewards
            episode = self.gen_eps(env, policy)
            Gt = 0

            # Definition of Epsilon decay
            eps = max(eps * eps_decay, 0.01)

            # View the rewards of the episode and the states in reverse order
            # and determine the respective returns
            for i in range(len(episode) - 1, -1, -1):

                state, action, reward = episode[i]
                # Calculating the return recursively
                Gt = reward + self.gamma * Gt

                if not (state, action, reward) in enumerate(episode[0:i]):
                    N[(state, action)] += 1
                    Q[(state, action)] += (Gt - Q[(state, action)]) / N[(state, action)]

                    # argmax provides the index of the max value of the list. According to the encoding this represents the action
                    best_action = np.argmax([Q[(state, a)] for a in range(env.action_space.n)])

                    for a in range(env.action_space.n):
                        if a == best_action:
                            policy[state[0], state[1], int(state[2])][a] = 1 - eps + eps / env.action_space.n
                        else:
                            policy[state[0], state[1], int(state[2])][a] = eps / env.action_space.n
