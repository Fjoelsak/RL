...
class TDEvalAgent:
    """
    TD(0) Evaluation Agent for estimating state values under a given policy.

    Attributes:
        gamma (float): Discount factor for future rewards.
        alpha (float): Step-size (learning rate) for TD updates.
    """

    def __init__(self, gamma, alpha):
        """
        Initialize the TD(0) Evaluation Agent.

        Args:
            gamma (float): Discount factor for future rewards.
            alpha (float): Step-size for TD updates.
        """
        self.gamma = gamma
        self.alpha = alpha
    ...
    def eval(self, env, n_episodes, policy):
        """
        Evaluate the given policy over multiple episodes using TD(0).
